...
import {
  allocateInventoryGeospatially,
  type InventoryAllocationResult,
} from '../allocate-inventory';
  ...
  describe('business logic', () => {
    test('When the nearest warehouse is missing an item, then it bypasses it and selects the farther fully-stocked warehouse', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouseA.id, productId: product1, quantity: 100, unitPrice: 1000 },
        { warehouseId: warehouseA.id, productId: product2, quantity: 100, unitPrice: 1000 },
        // Warehouse B has all 3
        { warehouseId: warehouseB.id, productId: product1, quantity: 50, unitPrice: 1000 },
        { warehouseId: warehouseB.id, productId: product2, quantity: 50, unitPrice: 1000 },
        { warehouseId: warehouseB.id, productId: product3, quantity: 50, unitPrice: 1000 },
      ]);
      ...
      // ASSERT
      expect(result.warehouseId).toBe(warehouseB.id);
    });
    ...
    test('When no single warehouse stocks all items (split-warehouse), then it throws a 422 AppError', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouseA.id, productId: productX, quantity: 100, unitPrice: 1000 },
        { warehouseId: warehouseB.id, productId: productY, quantity: 100, unitPrice: 1000 },
      ]);

      // ACT
      const promise = allocateInventoryGeospatially(
        [
          { productId: productX, quantity: 5 },
          { productId: productY, quantity: 5 },
        ],
    ...
    test('When multiple warehouses qualify, then it selects the closest one to the shipping location', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouseClose.id, productId, quantity: 20, unitPrice: 1000 },
        { warehouseId: warehouseFar.id, productId, quantity: 20, unitPrice: 1000 },
      ]);
      ...
      // ASSERT
      expect(result.warehouseId).toBe(warehouseClose.id);
    });
    ...
    test('When a single warehouse has a single product with sufficient stock, then it allocates and decrements correctly', async () => {
      ...
      // ACT
      const result = await allocateInventoryGeospatially([{ productId, quantity: 10 }], {
        lat: 40.7128,
        lng: -74.006,
      });
    ...
    test('When the nearest warehouse has insufficient quantity for one item, then it falls back to a farther warehouse', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouseClose.id, productId, quantity: 3, unitPrice: 1000 },
        { warehouseId: warehouseFar.id, productId, quantity: 50, unitPrice: 1000 },
      ]);
      ...
      // ASSERT
      expect(result.warehouseId).toBe(warehouseFar.id);
    });
    ...
    test('When a multi-item order is fulfilled, then each product quantity is decremented correctly', async () => {
      ...
      await db.insert(Inventory).values([
        ...
        { warehouseId: warehouse.id, productId: product2, quantity: 50, unitPrice: 1000 },
        { warehouseId: warehouse.id, productId: product3, quantity: 25, unitPrice: 1000 },
      ]);

      // ACT
      const result = await allocateInventoryGeospatially(
        [
          { productId: product1, quantity: 7 },
          { productId: product2, quantity: 3 },
          { productId: product3, quantity: 12 },
        ],
        { lat: 40.7128, lng: -74.006 },
      );
    ...
    test('When coordinates are at extreme boundary values, then it still processes correctly', async () => {
      ...
      const result = await allocateInventoryGeospatially([{ productId, quantity: 2 }], {
        lat: -44.5,
        lng: 179.5,
      });
    ...
    test('When stock is exactly equal to the requested quantity, then allocation succeeds with zero remaining', async () => {
      ...
      // ACT
      const result = await allocateInventoryGeospatially([{ productId, quantity: 7 }], {
        lat: 40.7128,
        lng: -74.006,
      });
    ...
    test('When no warehouses exist, then it throws a 422 AppError', async () => {
      ...
      // ACT
      const promise = allocateInventoryGeospatially([{ productId, quantity: 1 }], {
        lat: 40.7128,
        lng: -74.006,
      });
    ...
    test('When the first allocation depletes the closest warehouse, then the next allocation routes to the second closest', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouseClose.id, productId, quantity: 5, unitPrice: 1000 },
        { warehouseId: warehouseFar.id, productId, quantity: 10, unitPrice: 1000 },
      ]);

      const shippingLocation = { lat: 40.7128, lng: -74.006 };

      // ACT — first allocation depletes close warehouse
      const first = await allocateInventoryGeospatially(
        [{ productId, quantity: 5 }],
        shippingLocation,
      );

      // Second allocation must fall back to far warehouse (close is now at 0)
      const second = await allocateInventoryGeospatially(
        [{ productId, quantity: 3 }],
        shippingLocation,
      );

      // ASSERT
      expect(first.warehouseId).toBe(warehouseClose.id);
      expect(second.warehouseId).toBe(warehouseFar.id);
  ...
  describe('concurrency edge cases', () => {
    test('When two concurrent allocations compete for limited stock on a single warehouse, then one succeeds and the other throws 422', async () => {
      ...
      const results = await Promise.allSettled([
        allocateInventoryGeospatially([{ productId, quantity: 3 }], { lat: 40.7128, lng: -74.006 }),
        allocateInventoryGeospatially([{ productId, quantity: 3 }], { lat: 40.7128, lng: -74.006 }),
      ]);

      // ASSERT
      const fulfilled = results.filter(
        (r): r is PromiseFulfilledResult<InventoryAllocationResult> => r.status === 'fulfilled',
      );
      ...
      expect(fulfilled[0].value.warehouseId).toBe(warehouse.id);
    ...
    test('When three concurrent allocations contend for the closest warehouse but alternatives exist, then all succeed on different warehouses', async () => {
      ...
      // Each warehouse has exactly 5 units — enough for one order of quantity=5
      await db.insert(Inventory).values([
        { warehouseId: warehouseClose.id, productId, quantity: 5, unitPrice: 1000 },
        { warehouseId: warehouseMid.id, productId, quantity: 5, unitPrice: 1000 },
        { warehouseId: warehouseFar.id, productId, quantity: 5, unitPrice: 1000 },
      ]);
      ...
      const results = await Promise.allSettled([
        allocateInventoryGeospatially([{ productId, quantity: 5 }], shippingLocation),
        allocateInventoryGeospatially([{ productId, quantity: 5 }], shippingLocation),
        allocateInventoryGeospatially([{ productId, quantity: 5 }], shippingLocation),
      ]);
      ...
      const assignedWarehouseIds = fulfilled.map((r) => r.value.warehouseId).sort();
    ...
    test('When two concurrent orders request different products from the same warehouse, then both succeed without blocking each other', async () => {
      ...
      await db.insert(Inventory).values([
        { warehouseId: warehouse.id, productId: productA, quantity: 50, unitPrice: 1000 },
        { warehouseId: warehouse.id, productId: productB, quantity: 50, unitPrice: 1000 },
      ]);
      ...
      const results = await Promise.allSettled([
        allocateInventoryGeospatially([{ productId: productA, quantity: 10 }], shippingLocation),
        allocateInventoryGeospatially([{ productId: productB, quantity: 7 }], shippingLocation),
      ]);
      ...
      expect(fulfilled[0].value.warehouseId).toBe(warehouse.id);
      expect(fulfilled[1].value.warehouseId).toBe(warehouse.id);
